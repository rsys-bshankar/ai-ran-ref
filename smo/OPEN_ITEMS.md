...
1. Bring up the shallow-coverage modules (§4: so-smos, ran-analytics,
   focom) to parity with the rest.
2. Fill the call-flow gaps (§3) — mostly documentation, high value for
   onboarding new readers to the design.
3. Resolve the design-level decisions (§1) that block further code — SA
